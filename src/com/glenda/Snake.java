...
public class Snake {
	...
	//Numberos arbitrario representando a direcao (movimento) da cobriha
	...
	private int snakeSize;   //size of snake - how many segments? 2
	...
	protected void moveSnake(){
		...
		//	SnakeGame.setGameStage(SnakeGame.GAME_OVER);
			//return;
		//}
		//Cobrinha nao morre mais por encostar  na parede.
		if (ateTail) {
			SnakeGame.setGameStage(SnakeGame.GAME_OVER);
			return;
		}
		...
		if (currentHeading == DIRECTION_LEFT) {		
			...
			// snakeSquares[snakeHeadX-1][snakeHeadY]++;
			snakeHeadX -= 1; // snakeHeadX -- ;
		}
		...
		//Cobrinha passa direto de um lado para o outro (direita para esquerda e esqurda para direita)
		if (snakeHeadX >= maxX) //**
			snakeHeadX = 0;		// **
		...
		if (snakeSquares[snakeHeadX][snakeHeadY] != 0) {
			...
			ateTail = true;
			//Se cobrinha tocar a si mesma ela morre e o jogo acabou.
			SnakeGame.setGameStage(SnakeGame.GAME_OVER);
			return;
		}
